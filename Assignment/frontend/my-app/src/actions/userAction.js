...
import { USER_LOGIN_REQUEST, USER_LOGIN_SUCCESS, USER_LOGIN_FAIL,USER_LOGOUT, USER_DETAILS_REQUEST, USER_DETAILS_SUCCESS, USER_DETAILS_FAIL, USER_UPDATE_SUCCESS,USER_UPDATE_FAIL,USER_UPDATE_REQUEST,USER_UPDATE_RESET, USER_REGISTER_RESET, USER_LIST_REQUEST, USER_LIST_SUCCESS, USER_LIST_FAIL} from "../constants/userConstants";
...
export const listUsersSearch = (search, page) => async(dispacth,getState) => {
    try {
        dispacth({
            type: USER_LIST_REQUEST,
        })
        const {data} =  await axios.get(`http://localhost:4000/api/v1/getUserDetails?page=${page}&search=${search}`);
        console.log(data);
        dispacth({
